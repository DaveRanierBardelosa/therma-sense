...
const char* cloudServerUrl = "https://therma-sense-206b4-default-rtdb.asia-southeast1.firebasedatabase.app/telemetry.json?auth=lJZpD2eWpRfUs7NEnEnxEZnPE27UFn4rl24sZoTd";
  ...
  if (millis() - lastCloudPost >= cloudInterval) {
    ...
    if (isnan(t) || isnan(h)) {
      ...
      if (httpResponseCode == 404) {
        Serial.println("HTTP 404: Firebase Realtime Database not found at this URL.");
        Serial.println("Check that Realtime Database is enabled and the URL host is correct (try '-default-rtdb' form).");
